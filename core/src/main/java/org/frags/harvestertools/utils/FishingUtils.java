...
public class FishingUtils {
    ...
    public FishingUtils(HarvesterTools plugin) {
        this.plugin = plugin;
        this.enchantsManager = plugin.getEnchantsManager();
        this.configuration = plugin.rodEnchantsFile.getConfig();
    ...
    public void fastRod(PlayerFishEvent e, ItemStack itemStack) {
        CustomEnchant fastRod = enchantsManager.getEnchant("fastrod", Tools.rod);
        if (fastRod == null)
            return;
        ...
        int maxWaitTime = e.getHook().getMaxWaitTime();

        int reducedMinTime = (int) (minWaitTime * calculatedBoost);
        int reducedMaxTime = (int) (maxWaitTime * calculatedBoost);

        e.getHook().setMinWaitTime(reducedMinTime);
        e.getHook().setMaxWaitTime(reducedMaxTime);
    }

    public void calculateAutoSellDrops(ItemStack itemStack, Player player, ItemStack drop) {
        ...
        if (ToolUtils.isAutoSell(itemStack)) {
            ...
            if (!autoSellKey.contains(player)) {
                ...
                scheduler.runTaskLater(plugin, () -> {
                    ...
                    ConfigurationSection section = configuration.getConfigurationSection("CustomEnchants.autosell");
            ...
            addAmountToMaps(player, 0, initialEssencePrice, experience);
    ...
    public void procTsunami(Player player, ItemStack itemStack) {
        ...
        CustomEnchant moneyBooster = enchantsManager.getEnchant("moneybooster", Tools.rod);
        ...
        CustomEnchant essenceBooster = enchantsManager.getEnchant("essencebooster", Tools.rod);
        ...
        ConfigurationSection section = configuration.getConfigurationSection("CustomEnchants.tsunami");
        ConfigurationSection actionBar = section.getConfigurationSection("actionbar");
        if (actionBar.getBoolean("enabled")) {
            String message = MessageManager.miniStringParse(actionBar.getString("message"))
    ...
    public void calculateBoosters(Player player, ItemStack itemStack) {
        ...
        double moneyPrestigeBooster = container.getOrDefault(ToolUtils.moneyBoostKey, PersistentDataType.DOUBLE, 0D);
        double essencePrestigeBooster = container.getOrDefault(ToolUtils.essenceBoostKey, PersistentDataType.DOUBLE, 0D);
        CustomEnchant moneyBooster = plugin.getEnchantsManager().getEnchant("moneybooster", Tools.rod);
        if (moneyBooster != null) {
            if (enchantsManager.hasEnchantment(itemStack, moneyBooster)) {
        ...
        CustomEnchant essenceBoost = plugin.getEnchantsManager().getEnchant("essencebooster", Tools.rod);
        ...
        CustomEnchant experienceBoost = plugin.getEnchantsManager().getEnchant("experiencebooster", Tools.rod);
